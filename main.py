...
def main(page: ft.Page):
    ...
    def create_task_row(task_id, task_text):
        ...
        def set_completed(_):
            test = checkbox.value
            if test == 1 :
                main_db.update_completed(task_id,completed = 1)
            elif test == 0:
                main_db.update_completed(task_id , completed = 0)

        checkbox = ft.Checkbox(on_change=set_completed)
        ...
        return ft.Row([checkbox ,task_field, enable_button, save_button, del_button])
    ...
    del_all_button = ft.ElevatedButton('Удалить все покупки', on_click=del_all)
    task_input = ft.TextField(label='Введите название товара', expand=True, on_submit=add_task)
    add_button = ft.IconButton(icon=ft.Icons.ADD, tooltip='Добавить товар', on_click=add_task)
...
#ya ne stal menyat nazvaniya, sorry(